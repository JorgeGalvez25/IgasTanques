...
uses
  ...
  uLkJSON, Variants, IdHashMessageDigest, IdHash, ActiveX, ComObj, WinSvc,
  WinSock;
...
type
  ...
  public
    ...
    LineaBuff, LineaProc: string;
    ListaLog: TStringList;
    ListaLogPetRes: TStringList;
    SwTimer1: boolean;
    ContadorAlarma, ContadorAlarma3, ContadorAlarma2, ContRec, ContErroresCom, numPaso: integer;
    ...
    function IniciarServicio(const NombreServicio: string): Boolean;
    function ServicioCorriendo(const NombreServicio: string): Boolean;
    { Public declarations }
  end;
...
begin
  try
    try
      version := 'faa9333dc21bffaa1c18614388b58d69b5e262fa';
      ...
      servDisp := config.ReadString('CONF', 'ServicioDispensarios', '');
      WSAResult := WSAStartup($0202, WSAData);
      if WSAResult <> 0 then
        raise Exception.CreateFmt('WSAStartup failed with error: %d', [WSAResult]);
...
procedure Togcvtanques.ServerSocket1ClientRead(Sender: TObject; Socket: TCustomWinSocket);
var
...
begin
  try
    ...
    begin
      if UpperCase(ExtraeElemStrSep(mensaje, 1, '|')) <> 'TANKS' then
      begin
...
begin
  try
    ...
    if (TM_Tanques.RecordCount > 0) then
    begin
      Result := 'True|';
      ...
      TM_Tanques.First;
      while not TM_Tanques.Eof do
      begin
        if (tanqueInt = 0) or (tanqueInt = TM_TanquesCLAVE.AsInteger) then
        begin
...
begin
  ...
  if tipoTanques <> 6 then
    ...
    s1 := ss;
  if pSerial.OutBuffFree >= Length(s1) then
  begin
    AgregaLog('E ' + s1);
...
begin
  ...
  if (servDisp <> '') and (not ServicioCorriendo(servDisp)) then
  begin
    Sleep(1000);
    IniciarServicio(servDisp);
  end;
  ...
  begin
    case TipoTanques of
      1:
        ComandoConsola('i201' + inttoclavenum(TM_TanquesCLAVE.AsInteger, 2)); // VeederRoot
      2:
        ComandoConsola('E97' + inttostr(TM_TanquesCLAVE.AsInteger));  // EecoSystems
      3:
        ComandoConsola('10' + inttostr(TM_TanquesCLAVE.AsInteger));  // AutoStik
      4:
        ComandoConsola('i201' + inttoclavenum(TM_TanquesCLAVE.AsInteger, 2)); // Red Jacket
      6:
        begin      // Incon
...
var
  ...
  xFechaHoraI, xFechaHoraEnt: TDateTime;
begin
  try
    if LineaProc = '' then
      exit;
    lin := LineaProc;
    if (copy(lin, 1, 4) = 'i201') then
    begin // INVANTARIO TANQUES
      xtan := StrToIntDef(copy(lin, 5, 2), 0);
      if TM_Tanques.Locate('CLAVE', xtan, []) then
      begin
        lectRec := true;
        TM_Tanques.Edit;
    ...
    else if copy(lin, 1, 4) = 'i202' then
    begin // ENTRADAS AL TANQUES
      xtan := StrToIntDef(copy(lin, 5, 2), 0);
      if TM_Tanques.Locate('CLAVE', xtan, []) then
      begin
        TM_Entradas.First;
        while not TM_Entradas.Eof do
        begin
        ...
        begin
          ...
          if abs(xFechaHoraEnt - xFechaHoraI) < 2 then
          begin
            delete(line, 1, 22);
            for xval := 1 to 8 do
            begin
    ...
    begin
      TM_Tanques.Next;
      AgregaLog('Error procesar linea: ' + e.message);
    end;
  end;
end;
...
begin
  try
    if LineaProc = '' then
      exit;
    lin := LineaProc;
    if (copy(lin, 1, 3) = 'E97') then
    begin // INVANTARIO TANQUES
      lectRec := true;
      xtan := StrToIntDef(copy(lin, 4, 1), 0);
      if xtan = TM_TanquesCLAVE.AsInteger then
      begin
        TM_Tanques.Edit;
              ...
              case C of
                ...
                idETX:
                  begin
                    ...
                    if CheckSumVeederRoot(LineaProc) then
                    begin
                      ContadorAlarma2 := 0;
              ...
              case C of
                ...
                idETX:
                  begin
                    ...
                    ContadorAlarma2 := 0;
                    AgregaLog('R ' + LineaProc);
                    if (copy(Lineaproc, 1, 2) = '10') then
                    begin
                      if (copy(lineaproc, 46, 1) = '9') then
                      begin
                        ContRec := 0;
                        ContadorAlarma3 := 0;
                        ProcesaLinea3;
              ...
              case C of
                ...
                idETX:
                  begin
                    LineaProc := LineaBuff; // 815 7562 X 566833
                    if true then
                    begin //CheckSumVeederRoot(LineaProc) then begin
                      ContadorAlarma2 := 0;
                      AgregaLog('R ' + LineaProc);
                      ContRec := 0;
                      ProcesaLinea6;
...
begin
  try
    ...
    if TM_Entradas.RecordCount > 0 then
    begin
      entradas := TlkJSONList.Create;
      TM_Entradas.First;
      while not TM_Entradas.Eof do
      begin
        if (TM_EntradasTANQUE.AsInteger = tanque) or (tanque = 0) then
        begin
...
function Togcvtanques.IniciarServicio(const NombreServicio: string): Boolean;
var
...
begin
  ...
  if SCManager > 0 then
  begin
    Service := OpenService(SCManager, PChar(NombreServicio), SERVICE_START);
    if Service > 0 then
    begin
      if StartService(Service, 0, x) then
      begin
        // Espera a que el servicio esté en ejecución
...
function Togcvtanques.ServicioCorriendo(const NombreServicio: string): Boolean;
var
  SCManager, Service: SC_Handle;
  ServiceStatus: TServiceStatus;
begin
  ...
  if SCManager > 0 then
  begin
    Service := OpenService(SCManager, PChar(NombreServicio), SERVICE_QUERY_STATUS);
    if Service > 0 then
    begin
      if QueryServiceStatus(Service, ServiceStatus) then
        Result := (ServiceStatus.dwCurrentState = SERVICE_RUNNING) or (ServiceStatus.dwCurrentState = SERVICE_CONTINUE_PENDING) or (ServiceStatus.dwCurrentState = SERVICE_START_PENDING);
      CloseServiceHandle(Service);
    end;
    CloseServiceHandle(SCManager);
  end;
